...
gzip processed_data/lobster.012
```
...
gzip processed_data/anchovy.012
```
...
gzip processed_data/dolphin.012
```
...
This data set is in Genepop format, which is an abominable format for large SNP data sets.
On top of that, it had a weird special character or an extra column in each row.  Processing
it was a bit of a hassle, and I don't show that here.   But the 012 file is in `raw_data`.
...
gzip processed_data/chinook_gatk.012
```
...
gzip processed_data/chinook_hecht.012
```
...
And now we just have to move those so that the don't have the junk on them.
```{sh}
for i in processed_data/*_indv*; do j=${i/_indv*./.}; mv $i $j; done 
```
...
gzip processed_data/snails.012
```
...
tibble(sample = rownames(dat)) %>%
  ...
  write_csv(., path = "processed_data/snail_pops.csv")
```


```{r, eval=FALSE}
gfc <- geno_freq_calcs(dat, pops = read_csv("processed_data/snail_pops.csv"))
